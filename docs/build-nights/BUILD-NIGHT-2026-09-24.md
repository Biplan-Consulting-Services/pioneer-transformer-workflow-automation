...
**2026-09-24 22:5x | `claude-43` |** Live-checked MEG / dates / LDs from the mirror (22:25 snapshot, before RUN2), results in the checklist. The user confirms **they** repointed E21007-1/1's lookups (before this investigation; still to confirm with the users). Dates and LDs are now on the user's to-do for 09-25. Added **E8c** (catalog fill-rate and key flag) after I searched `Order.Title`, which is empty; the order number is `Order_x0020_Number1`.

**2026-09-24 22:5x | `claude-43` |** User answered E10 D1–D6 (design §0): D1 nightly+2h but **deferred** to dedicated infrastructure; D2 30 d + monthly; D3 snapshots out of git, **latest in git**, and a separate `live/` workbook folder; D4 completion records later from the journal; D5 25 rows **plus a parent-fan-out classifier**; D6 raise Order Items + Order to 500 (user action, on the checklist). E10 build steps 1–4 unblocked.

**2026-09-24 23:1x | `claude-43` | RUN2 APPLIED AND VERIFIED FROM THE MIRROR.** x22 OVERWRITE with `window.x25OV` (748 fields / 700 writes / 696 units). x22's read-back crashed on a **503** from SharePoint (throttling after ~700 writes), so it never printed its tally. Verified instead by a fresh mirror refresh (23:11, 35 s) diffed against the pre-RUN2 22:25 set, using the `Columns` catalog's synced-from mapping (47 fields):
- Order Items cells changed: **748**, on **696** units; outside the synced fields: **0**; rows added/deleted: **0**.
- Drifted pairs fixed **748/748**; newly drifting **0**. Parent drift **822 → 74**: D5 dates 32, D6 LDs/EngReq (Order blank) 36, E21007 6.
...
**2026-09-25 01:1x | `claude-43` |** **Nightly Sync v003 STAGED** (`99c27f3`, reviewed, 14/14 mutation tests). C1 = Livraison AND Delivered; Excel date >= 7 d as the clock; new report-only lists of units Excel and Order Items disagree on (C4c) and Excel-done units with no row left (C4d). Dry run, cap 50. v002 superseded unpasted. **E12** assigned to `claude-5b`: port x25 / x16 / x17 into `mirror_health.py`, with today's known findings acknowledged so only NEW problems show red.
