...
| 40 | 🔴 **A run-scoped variable leaks across rows — SA units get the previous row's model** | 🔴 A | **new 2026-09-08, code review** | `ModelIDToWrite` / `ModelRevisionIDToWrite` are initialised **once before the loop**, and on the `SAModelUnmatched` path (SA unit with no twin) **neither is set** — so the write uses whatever the *previous iteration* left there. The unit is silently attached to another order's model, deterministically (the loop is sequential). `SAModelUnmatched` appears **once** in the definition: defined, never read — it stops nothing. Only **15 of 390** models have a twin, so the no-twin path is the common case for the 42 SA units, not an edge case. **Fix: null both variables at the top of every iteration** — then no-twin writes an empty lookup, visibly wrong instead of confidently wrong. `transfer-flow-code-review-2026-09-08.md` §1 |
| 41 | 🔴 **Two `If`s have no `else`, so rows vanish untraceably** | 🔴 A | **new 2026-09-08, code review** | `CheckOrderMatch` (`length(Get_Orders) == 1`) skips **the whole row** when false — no `Order Items` write, no log, no count. So "982 iterations completed" is **not** a claim that 982 rows were written. `CheckMatchCountModels` likewise skips the `Model Revisions` update. Strong candidate for part of the missing-row diff — and the six named survivors look exactly like units whose orders are absent from `Order`, which is **testable against the exports**. Fix: an `else` on each appending the unit ID to an array variable |
| 42 | 🟢 **`GetResolvedOrder` is redundant — 1,019 calls for nothing** | 🟢 A | **new 2026-09-08** | `Get_Orders` is a Get items with **no `$select`**, so it already returns every field `GetResolvedOrder` re-fetches by ID. Replace `outputs('GetResolvedOrder')?['body/X']` with `first(outputs('Get_Orders')?['body/value'])?['X']` and delete the action. Free, independent of everything else |
...
| 38 | **`Order - Order Folder` (URL) left out of v005** | 🟢 B | **new 2026-09-08** | The other 18 `Ord*` columns are populated by v005, but a SharePoint **hyperlink** column is an object on both read and write and I could not establish the shape the connector expects from anything in this repo. A wrong shape either fails the row — `Action 'Switch' failed`, across 1,019 rows — or writes nothing. It is one folder link on a column no staff view shows, so guessing was not worth it. **Confirm the shape from the D4 smoke test's raw inputs, then add it** |
| 39 | **Populate `Model - X` (5) and `Mod. Rev. - X` (24) in a later run** | 🟡 A | **new 2026-09-08** | Deliberately not in v005. `Models`: `GetModels1` runs on a **parallel branch** to the write path (both hang off `ItemStatus`), so referencing it from the writes is not guaranteed to have completed — in Logic Apps that yields **null, silently**. Needs the branches serialised first, which is a structural edit not a mapping. `Model Revisions`: the loop holds only the revision's **ID**, never the record — needs a new `Get item` per row, **~1,019 extra connector calls** on a tenant where request quota is already a live concern. Both are safe to do; neither is free |
