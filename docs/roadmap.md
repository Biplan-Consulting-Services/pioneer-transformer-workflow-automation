...
| 17 | **Centralise parents into `Order Items`** — build sheets ready | 🔵 A | **specced 2026-09-05** | 55 Yes / 62 No of 117 (**48** real new columns after the ten post-picker overrides — see `n2-column-build-sheet.md`). Naming revised: `Model`/`Model Revision` are **not** prefixed — they overwrite the existing lookups. SA units **re-resolve**, never copy. Change-guard on every write |
...
| 26 | ~~**FRM11 — how tightly is it coupled to FRM10-12?**~~ | ✅ A | **ANSWERED 2026-09-06** | **Very tightly, and worse than "a sync": `TableOrders` is FRM11's root query.** Ten columns feed `Imported FRM10_12 Data` → `FournTank` → 890 live rows → 8 supplier report sheets → 8 outside companies. It also imports the three tank-code tables from FRM10-12, and its purge rule reads FRM10-12's `Location`/`Status` **in two-letter codes**. Resolution goes through a SharePoint list named `Index` (Title → Path), which is why the 2026-09-04 workbook move did not break it — **and which makes the eventual cutover a one-row edit**. Full analysis, the 10-column list and the row-set reconciliation in `frm11-coupling-analysis-2026-09-06.md`; M code tracked at `power-query/FRM11/*.pq`. **Action: do not reshape or retire `TableOrders`.** One check left — that the `Index` row for `FRM10-12` points at `Revue/Formulaires/` |
| 27 | ~~**Which tank-status vocabulary is current?**~~ | ✅ A | **ANSWERED 2026-09-06 — the question was malformed** | **Both are, and they are different vocabularies on different axes.** `Status` comes from FRM10-12's `Tank` code via the three `List`-sheet code tables — which FRM11 *imports* from FRM10-12, so they cannot drift. `Tank Supplier Status` / `Paint Supplier Status` are **computed from the supplier reports** (`TableLocales` marks them `Calculated`), which is where `Dessin reçu` / `BC reçu` / `Terminée` come from. Nothing to reconcile, nobody to ask. ⚠️ Worth knowing: `Status` is populated on **15 of 890** rows — the `Statut Cuve` vocabulary is effectively unused, only `R` ever appears |
| 28 | **`Order Items.Tank` throws away the tank status code** | 🟡 B | **new 2026-09-06** | In `TableOrders`, `Tank` holds a two-letter status code (`R` = Reçu Pioneer on 74 rows). The transfer flow maps it `@not(equals(item()?['Tank'], ''))` — a Yes/No, `True` on 143 of 1,052. The vocabulary never reaches SharePoint. Harmless *today* because `R` is the only code in use, so `Tank = true` happens to mean "Reçu Pioneer" — but it stops being true the moment someone types a different code. One expression change carries it as text; only worth doing if anyone intends to use `Statut Cuve`, which today nobody does |
| 30 | **Cut the FRM10-12 ⇄ FRM13 lead-time cycle** | 🟡 A | **new 2026-09-06** | The estate has a real cycle: FRM13's `TableOrders` reads FRM10-12's `TableOrders`, and FRM10-12's `ClientLeadTimes` reads FRM13's hand-maintained `LeedTime` back through the `Index` (title `FRM13-Auto`). There is **no refresh order that makes the estate consistent**, and this is the likely mechanism behind `Order.Lead Time` disagreeing with FRM13's `LeedTime` on **306 of 342** rows — stale hop, not overrides. Moving `LeedTime` onto the `Clients` list (already planned) removes the cycle. See `workbook-data-graph-2026-09-06.md` |
| 31 | **The `Index` list is a single point of failure** | 🟡 A | **new 2026-09-06** | Every cross-workbook read in all five workbooks goes through one SharePoint list (`Title` → `Path`) — **no hardcoded workbook URL exists anywhere**. That is why the FRM10-12 move broke nothing, and it makes the eventual cutover a one-row edit. It is also unversioned and unwatched, and a comma in a `Path` breaks the hyperlink-stripping. Get it into whatever gets backed up |
| 32 | **Power Query helpers are copy-pasted across workbooks and have drifted** | 🟢 B | **new 2026-09-06** | `ImportFromIndex`, `ReplaceAllErrors`, `Index`, `TrackRemoteTable`, `EnforceFormats`, `AddAddedTimestamps` exist as independent copies in each workbook — FRM13 carries two copies of three of them (`Index (2)` etc.). They have already drifted (`ImportFromIndex` is 288 / 315 / 329 chars in different files). Nobody can fix a helper once. Now that `Export-PowerQuery.ps1` and `Sync-PowerQuery.ps1` round-trip, a shared `power-query/_shared/` set is cheap — **but it edits live workbooks, so ask first** |
...
| 35 | ~~**D6 is already built — and it erases as well as fills**~~ | 🔴 A | **new 2026-09-07** | `item/Family/Value: @item()?['Family']` is **already mapped** on the flow's `Model Revisions` update, so D6 is not "add the fill pass". It has **no blank guard**, and 385 of 1,019 unit rows have a blank `Family` — so a blank source clears a revision's value, which is the likely reason `Family` is blank on 329 of 391 despite the mapping running. Fix is a **Condition** (null clears a Choice too, so the key must not be sent). ✅ The tracker's fill-in gate is **closed**: all source values are inside `{A,B1,B2,C}`, zero outside. ⚠️ Confirm from one page of run history before changing anything. Also 2 models whose units disagree (`132125` C×22/A×2, `1001912-22` C×6/A×1) — majority `C` in both, but it is a call. Detail in `transfer-flow-forensics-2026-09-04.md` §13 |
| 34 | **Three `int()` date columns have no `EC` guard at all** | 🟡 B | **new 2026-09-07** | `ManualEstimatedDeliveryDate`, `OriginalTankingDate` and `TankDeliveryDate` call `int()` inside `addDays` with only an empty-string guard — no `EC` branch, so A5c's `toLower` fix does not apply to them; they need a guard added. **Clean today** (`scripts/ec_scan.py` checks all three), so this is hardening, not a blocker — but the same argument A5c makes holds: the markers are staff typing into a live column, not a fixed legacy set. Left out of the A5c paste sheet on purpose to keep that paste mechanical |
| 44 | 🔴 **The tracked `TableOrders.pq` is stale and would break the workbook if synced** | 🔴 A | **new 2026-09-08** | `FRM10-12/power-query/TableOrders.pq` removes **7** columns including `"Archived"`; the query actually inside the workbook removes **6** and does not mention it. `Archived` was dropped from the sheet table on 2026-08-29 (FRM10-12 WS-011, "static cached text, no live formula") and the workbook's query was fixed — **the repo copy never was**. Confirmed: the live table has 82 columns and no `Archived`. `Table.RemoveColumns` without `MissingField.Ignore` **throws** on a missing column, so running `Sync-PowerQuery.ps1 -Apply` from the repo would install a query that fails on the next refresh — on the workbook the transfer run reads. The workbook is fine today; the repo is the landmine. **Fix in the FRM10-12 repo** (drop `"Archived"`, or add `MissingField.Ignore`), and consider having `Export-PowerQuery.ps1` diff repo-vs-workbook so this drift is visible. Found by the view-design agent |
| 40 | ~~🔴 **A run-scoped variable leaks across rows — SA units get the previous row's model** | 🔴 A | **✅ FIXED in v006, live 2026-09-08** | `ModelIDToWrite` / `ModelRevisionIDToWrite` are initialised **once before the loop**, and on the `SAModelUnmatched` path (SA unit with no twin) **neither is set** — so the write uses whatever the *previous iteration* left there. The unit is silently attached to another order's model, deterministically (the loop is sequential). `SAModelUnmatched` appears **once** in the definition: defined, never read — it stops nothing. Only **15 of 390** models have a twin, so the no-twin path is the common case for the 42 SA units, not an edge case. **Fix: null both variables at the top of every iteration** — then no-twin writes an empty lookup, visibly wrong instead of confidently wrong. `transfer-flow-code-review-2026-09-08.md` §1 |
| 41 | ~~🔴 **Two `If`s have no `else`, so rows vanish untraceably**~~ | 🔴 A | **✅ FIXED in v006, live 2026-09-08** | `CheckOrderMatch` (`length(Get_Orders) == 1`) skips **the whole row** when false — no `Order Items` write, no log, no count. So "982 iterations completed" is **not** a claim that 982 rows were written. `CheckMatchCountModels` likewise skips the `Model Revisions` update. Strong candidate for part of the missing-row diff — and the six named survivors look exactly like units whose orders are absent from `Order`, which is **testable against the exports**. Fix: an `else` on each appending the unit ID to an array variable |
| 42 | ~~🟢 **`GetResolvedOrder` is redundant**~~ | 🟢 A | **new 2026-09-08** | `Get_Orders` is a Get items with **no `$select`**, so it already returns every field `GetResolvedOrder` re-fetches by ID. Replace `outputs('GetResolvedOrder')?['body/X']` with `first(outputs('Get_Orders')?['body/value'])?['X']` and delete the action. Free, independent of everything else |
| 43 | ~~🟡 **Fetch each list once instead of 9 calls per row**~~ | 🟡 A | **new 2026-09-08, user's proposal — validated** | Cache `Order`/`Models`/`Model Revisions`/`Order Items` before the loop and replace the per-row queries with `Filter array`, as `Filter_BO` already does. **9 calls/row → 3**: ~9,171 → **~3,063 per run**, and it makes the 24 `Rev*` columns **free** instead of +1,019 calls. ✅ The snapshot risk was checked, not assumed: `Order Items` is written during the run, but **1,019 source rows have 1,019 distinct Titles**, so no iteration can need a row an earlier one created. ⚠️ One real behaviour change — 5 of the 18 `Ord*` sources are also written by `UpdateOrder`; recommend sourcing those five from **Excel** instead, removing the ordering dependency entirely. ⚠️ Every cached fetch needs `paginationPolicy: 5000` — Get items returns **100 by default**, and the four existing in-loop queries have neither `$top` nor pagination today (safe only because each filters to 0–1 rows) |
| 38 | **`Order - Order Folder` (URL)** | ✅ DONE | **closed 2026-09-10** | Was left out of v005 because a SharePoint **hyperlink** column is an object on both read and write and the shape could not be established from anything in this repo. Closed two ways: `scripts/x5_backfill_order_folder.js` copies the existing 106 over REST with `SP.FieldUrlValue` (not bound to the cutover window — its source is the `Order` list), and N3's Order flow now carries `OrdOrderFolder` for anything new (18 → 19 fields). The connector **write** shape is sourced — it renders a URL column as one input box, so it takes a bare string. The **read**, `?['Url']`, is still reasoned rather than observed; `docs/n3-deploy-and-test.md` Test C settles it on one row. Deliberately **not** added to the transfer flow, which is deleted at cutover. |
| 39 | ~~**Populate `Model - X` (5) and `Mod. Rev. - X` (24)**~~ | 🟡 A | **new 2026-09-08** | Deliberately not in v005. `Models`: `GetModels1` runs on a **parallel branch** to the write path (both hang off `ItemStatus`), so referencing it from the writes is not guaranteed to have completed — in Logic Apps that yields **null, silently**. Needs the branches serialised first, which is a structural edit not a mapping. `Model Revisions`: the loop holds only the revision's **ID**, never the record — needs a new `Get item` per row, **~1,019 extra connector calls** on a tenant where request quota is already a live concern. Both are safe to do; neither is free |
| 37 | ~~**N2 — create the 48 parent columns**~~ | ✅ A | **DONE 2026-09-07** | All 48 created on `Order Items` and verified by read-back: 117→165 columns, 48 in group `Parent Sync` (19 `Ord` / 5 `Mdl` / 24 `Rev`), every internal name honoured exactly, **zero** Choice or Lookup stored, 3 DateTime all `dateOnly`, and **0 of 48 leaked into the default view**. All empty — no data changed, no flow fired. Two script defects fixed first: the read-back *and the UNDO* used a dead endpoint, so the rollback would have silently no-opped. Detail in `n2-column-build-sheet.md` |
...
| 45 | **Clients → Order lead-time sync** | 🟡 C | **new 2026-09-10, deferred by the user to after cutover** | `n6_order_lead_time.js` was a ONE-TIME backfill: it set `Order.Lead Time` from the client on 366 orders, and all 412 now agree. But nothing keeps it that way. The `Clients - Create or Update Trigger` flow fans out to **Order Items only**, so if a client's lead time changes, `Order.Lead Time` drifts straight back to the stale state this fixed — which is exactly how the 91 wrong values arose in the first place (a one-time fill at order creation, never re-synced). Fix is a second fan-out in that flow: `Get items` on `Order` filtered `ClientId eq <trigger ID>`, guard on `Lead_x0020_Time`, write it. That is a structural change to `gen_n3_flows.py`, which today only ever writes to Order Items — hence deferred rather than rushed before the freeze. Interim: re-run n6, two minutes. |
