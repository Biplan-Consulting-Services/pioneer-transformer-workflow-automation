...
| D6 | **Version counts, every item counted (mirror `VersionCounts`, 22:35).** History is **already being lost**. Items over the 50-version limit: **81** units, **30** orders, **25** revisions, **3** models. Medians are 5–17; p95 is 42–61. 🔴 Outliers that look like **write loops**: unit **126 = `21792-3/5`, 851 versions** since 08-17 (~25 a day, last 21:45 local tonight), and revision **26, 510 versions**. A loop would eat any new limit, so it is being traced first. Raising the limit does not bring lost history back, **so snapshots are needed as the undo record now, not later.** | 500 covers p95 but not the loops. **Find and stop the loop(s) first**, then raise to 500 |
| D6a | **Raise the version limit on Order Items?** Step 0 (22:31, mirror `Lists` table): all 7 lists have versioning ON, but at the SharePoint default of **50 major versions**. Every fan-out, trigger run, x22 write and staff edit adds a version to a unit, so a busy unit can lose its oldest history, and with it the fine-grained undo this design falls back on. | raise **Order Items to 500** (and Order to 500), a one-setting change in list settings. Decide after the read-only check of how close units already are to 50. Until then, snapshots are the primary undo record |
